...
#SBATCH --job-name=physics_playback         # Nombre del trabajo
#SBATCH --time=08:00:00                     # Aumentamos el límite de tiempo a 8 horas por seguridad
#SBATCH --cpus-per-task=8                   # 8 núcleos de CPU para carga de datos
#SBATCH --gpus=1                            # 1 GPU GTX 1080Ti
#SBATCH --mem=16G                           # 16 GB de memoria RAM
...
apptainer exec --nv --bind /data/emunoz:/data /data/emunoz/imgs/python_full.sif /data/emunoz/envs/dev_env/bin/python src/0_data_playback_physics_collection.py