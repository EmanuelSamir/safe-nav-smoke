...
def main():
    ...
    # Print comparison
    print(f"{'Dataset Name':<42} | {'Type':<20} | {'Ep/Items':<8} | {'Steps':<6} | {'Grid':<8} | {'MaxVal':<6} | {'NonZero%':<8}")
    print("-" * 115)
    ...
    for r in results:
        ...
        print(f"{r['name']:<42} | {r['type'][:20]:<20} | {r['num_episodes']:<8} | {r['episode_steps']:<6} | {r['grid_shape']:<8} | {r['max_value']:<6.2f} | {r['nonzero_fraction']*100:<7.1f}%")
        
        # Candidate selection logic:
        # Must have smoke_data, non-zero values, and substantial episodes
        if r["type"] in ["HuggingFace Dataset", "NumPy NPZ"] and r["num_episodes"] > 0 and r["nonzero_fraction"] > 0.01:
