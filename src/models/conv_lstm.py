...
class ConvLSTMModel(nn.Module):
    def __init__(self, cfg: ConvLSTMConfig):
        ...
        # MLP decoder (B, H, W, C_post) → (B, H, W, 2*h_pred) or (B, H, W, h_pred)
    ...
    def _build_feat_volume(
        self, frames: torch.Tensor, times: Optional[torch.Tensor]
    ) -> torch.Tensor:
    ...
    def forward(self, frames: torch.Tensor, times: Optional[torch.Tensor] = None) -> list:
        """Frames : (B, h_ctx, H, W) context.

        times  : (B, h_ctx) relative times
        returns: List[Normal] or List[torch.Tensor] len h_pred.
        """
            ...
            if not self.cfg.is_probabilistic:
                ...
                for d in out_forward:
                    ...
                    if mode == "mean":
                        ...
                        eps = torch.randn(
                            d.mean.shape[0], 1, 1, 1, device=d.mean.device, dtype=d.mean.dtype
                        )
...
def main():
    ...
    print(f"ConvLSTM params: {n:,}  (h_ctx={conv_cfg.h_ctx}, h_pred={conv_cfg.h_pred})")

    B, H, W = 2, 20, 30
    frames = torch.randn(B, conv_cfg.h_ctx, H, W)
    times = torch.linspace(0, 0.4, conv_cfg.h_ctx).unsqueeze(0).expand(B, -1)

    dists = model(frames, times)
    assert len(dists) == conv_cfg.h_pred
    assert dists[0].mean.shape == (B, H, W, 1)
    print(f"Forward OK  h_pred={conv_cfg.h_pred}  mu shape={dists[0].mean.shape}")

    seed = torch.randn(1, conv_cfg.h_ctx, H, W)
    ...
    print("Testing ConvLSTMConfig validation constraints...")
    try:
        ConvLSTMConfig(
            h_ctx=5,
            h_pred=5,
        ...
        raise RuntimeError("Validation failed to raise error for even kernel_size")
    except ValueError:
        print("Validation error raised correctly for even kernel_size")
