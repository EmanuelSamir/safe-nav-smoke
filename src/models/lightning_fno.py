...
def make_times(t_offset: int, h_ctx: int, seq_len: int, device, B: int) -> torch.Tensor:
    ...
    Absolute times of the window [t_offset .. t_offset+h_ctx-1] scaled to
    [0,1] relative to the full sequence length (seq_len).
    """
...
class FNODataModule(L.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        # Expose spatial size attributes
...
class FNOLightningModule(L.LightningModule):
    def __init__(
        self, t_cfg: FNOTrainingConfigSchema, H: int, W: int, x_size: float, y_size: float
    ):
        super().__init__()
