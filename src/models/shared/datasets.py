...
class BaseSmokeDataset(Dataset):
    ...
    def __init__(
        ...
        if os.path.isdir(data_path):
            ...
            ds = load_from_disk(data_path).with_format("numpy")
            self.smoke_data = ds[:]["smoke_data"].astype(np.float32)
            self.dt = float(ds["dt"][0])
            self.x_size = float(ds["x_size"][0])
            self.y_size = float(ds["y_size"][0])
            self.res = float(ds["resolution"][0])
        else:
...
class NonSequentialDataset(BaseSmokeDataset):
    ...
    def __init__(
        ...
        assert self.sequence_length < self.n_steps, (
            f"Sequence length {self.sequence_length} must be less than number of steps {self.n_steps}"
        )
        if self.context_length is not None:
            assert self.context_length <= self.sequence_length, (
                "Context length must be <= Sequence length"
            )

        assert trg_points_ratio > max(ctx_points_ratios), (
            "Target points ratio must be greater than max context points ratio"
        )
    ...
    def _sample_points(self, episode, time_indices, num_pts, t_offset: float = 0.0) -> Obs:
        ...
        if self.normalized_coords:
            ts = (
                2.0 * torch.from_numpy(t_idx * self.dt - t_offset).float() / self.max_diff_time
                - 1.0
            )
        else:
...
class SequentialDataset(BaseSmokeDataset):
    ...
    def __init__(
        ...
        super().__init__(
            data_path, train_split, mode, max_episodes, downsample_factor=downsample_factor
        )
        self.ctx_min_pts = int(self.H * self.W * ctx_points_ratios[0])
        self.ctx_max_pts = int(self.H * self.W * ctx_points_ratios[1])
        self.trg_pts = int(self.H * self.W * trg_points_ratio)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(data_path):
        ...
        dataset = NonSequentialDataset(
            data_path, sequence_length=25, context_length=10, max_episodes=5
        )
        ...
        sc1 = axs[0, 0].scatter(
            xs_c[mask_c_t0], ys_c[mask_c_t0], c=vs_c[mask_c_t0], cmap="viridis", s=5
        )
        axs[0, 0].set_title(f"Context t={ts_c_t0} (Valid: {mask_c_t0.sum()})")
        plt.colorbar(sc1, ax=axs[0, 0])

        sc2 = axs[0, 1].scatter(
            xs_c[mask_c_tm1], ys_c[mask_c_tm1], c=vs_c[mask_c_tm1], cmap="viridis", s=5
        )
        ...
        sc2 = axs[1, 0].scatter(
            xs_t[mask_t_t0], ys_t[mask_t_t0], c=vs_t[mask_t_t0], cmap="viridis", s=5
        )
        axs[1, 0].set_title(f"Target t={ts_t_t0} (Valid: {mask_t_t0.sum()})")
        plt.colorbar(sc2, ax=axs[1, 0])

        sc3 = axs[1, 1].scatter(
            xs_t[mask_t_tm1], ys_t[mask_t_tm1], c=vs_t[mask_t_tm1], cmap="viridis", s=5
        )
        ...
        seq_loader = DataLoader(
            seq_dataset, batch_size=8, collate_fn=sequential_collate_fn, shuffle=True
        )
        ...
        sc1 = axs[0, 0].scatter(
            xs_c0[mask_c0], ys_c0[mask_c0], c=vs_c0[mask_c0], cmap="viridis", s=5
        )
        ...
        sc1 = axs[0, 1].scatter(
            xs_cm1[mask_cm1], ys_cm1[mask_cm1], c=vs_cm1[mask_cm1], cmap="viridis", s=5
        )
        ...
        sc2 = axs[1, 0].scatter(
            xs_t0[mask_t0], ys_t0[mask_t0], c=vs_t0[mask_t0], cmap="viridis", s=5
        )
        ...
        sc2 = axs[1, 1].scatter(
            xs_tm1[mask_tm1], ys_tm1[mask_tm1], c=vs_tm1[mask_tm1], cmap="viridis", s=5
        )
        ...
        ds_down = NonSequentialDataset(
            data_path, sequence_length=25, max_episodes=1, downsample_factor=2
        )
