...
class NonSequentialDataset(Dataset):
    """
    Dataset for global smoke forecasting using for non-sequential
    models like CNP-PINN. Loads data from a 4D tensor
    (Episodes, Time, H, W).
    """
    def __init__(self, 
                 data_path: str,
    ...
    def __getitem__(self, idx: int) -> Tuple[ObsPINN, ObsPINN, ObsPINN, torch.Tensor, int]:
        ...
        ctx_obs = self._sample_points(episode, frame_indices, self.num_ctx_pts_per_frame, t_offset=t_offset)
        trg_obs = self._sample_points(episode, frame_indices, self.num_trg_pts_per_frame, t_offset=t_offset)
        ...
        return ctx_obs, trg_obs, total_obs, idx, t_offset
