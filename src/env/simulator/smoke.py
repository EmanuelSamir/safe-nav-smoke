...
class SmokeParams:
    x_size: float = 30.0
    y_size: float = 30.0
    smoke_blob_params: list[BlobParams] = field(default_factory=list)
...
class Smoke:
    def __init__(self, params: Optional[SmokeParams] = None):
        self.params = params
    ...
    def build_smoke_map(self, smoke_blob_params: list[BlobParams]):
    ...
    def get_smoke_density_at_point(self, x, y) -> float:
        """Function to get smoke density at a point.
    ...
    def get_smoke_density(self, pos: np.ndarray) -> np.ndarray:
        """Function to get smoke density at multiple points.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Test Dynamic Smoke")
    parser.add_argument("--world_x_size", type=int, default=60)
    parser.add_argument("--world_y_size", type=int, default=50)
    ...
    world_x_size = args.world_x_size
    world_y_size = args.world_y_size
    sensor_params = DownwardsSensorParams(world_x_size=world_x_size, world_y_size=world_y_size)
    smoke_params = SmokeParams(
        x_size=world_x_size,
        y_size=world_y_size,
        smoke_blob_params=smoke_blob_params,
        resolution=args.resolution,
    )
    smoke_simulator = Smoke(params=smoke_params)
