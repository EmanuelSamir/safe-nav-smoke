"""
train_fno_3d_decoupled.py — Training for Decoupled FNO-3D
=========================================================
Implements a scheduled training strategy where the mean and std
networks can be trained independently (mean first, then std, then joint).
"""
...
@hydra.main(version_base=None,
            ...
            config_name="fno_3d_decoupled_train")
def train(cfg: DictConfig):
    print(f"Training FNO-3D Decoupled — {cfg.training.experiment_name}")
    ...
    fno_cfg = FNO3dDecoupledConfig(
        h_ctx        = h_ctx,
        h_pred       = h_pred,
        
        modes_t_mu   = mc.get("modes_t_mu",  4),
        modes_h_mu   = mc.get("modes_h_mu",  8),
        modes_w_mu   = mc.get("modes_w_mu",  8),
        width_mu     = mc.get("width_mu",   32),
        n_layers_mu  = mc.get("n_layers_mu", 4),
        
        modes_t_std  = mc.get("modes_t_std",  2),
        modes_h_std  = mc.get("modes_h_std",  4),
        modes_w_std  = mc.get("modes_w_std",  4),
        width_std    = mc.get("width_std",   16),
        n_layers_std = mc.get("n_layers_std", 4),
    ...
    model = FNO3dDecoupled(fno_cfg).to(device)
    print(f"FNO3dDecoupled params: {sum(p.numel() for p in model.parameters()):,}")

    # ---- Optimizer & Schedule -----------------------------------------------
    opt_cfg   = cfg.training.optimizer
    sched_cfg = cfg.training.schedule
    
    epochs_mean_only = sched_cfg.get("epochs_mean_only", 50)
    epochs_std_only  = sched_cfg.get("epochs_std_only", 50)
    epochs_joint     = sched_cfg.get("epochs_joint", 100)
    max_epochs       = epochs_mean_only + epochs_std_only + epochs_joint
    
    optimizer = optim.Adam(model.parameters(), lr=opt_cfg.lr)
    scheduler = optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=max_epochs, eta_min=opt_cfg.min_lr)
    ...
    for epoch in range(max_epochs):
        ...
        if epoch < epochs_mean_only:
            phase = "mean_only"
        elif epoch < epochs_mean_only + epochs_std_only:
            phase = "std_only"
        else:
            phase = "joint"
        ...
        pbar = tqdm(train_loader, desc=f"Epoch {epoch} [{phase}]")
        for batch in pbar:
