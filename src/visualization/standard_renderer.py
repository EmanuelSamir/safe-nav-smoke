...
class StandardRenderer(BaseRenderer):
    ...
    def render(self, info: Dict[str, Any]):
        ...
        if predicted_maps and "pred" in self.axes:
            if builder is not None:
                ...
                if len(coords) == len(map_data):
                    ...
                    if H * W == len(map_data):
                        ...
                        print(
                            f"Warning: Map data length ({len(map_data)}) doesn't match grid HxW ({H}x{W})."
                        )
            ...
            if pred_mappable and self.cbar_map.get("pred") is None:
                ...
                self.cbar_map["pred"].set_label(
                    f"Predicted Risk Map at horizon = {horizon}", fontsize=12
                )
        ...
        if hasattr(self.axes["env"], "images") and self.axes["env"].images:
            ...
            if self.cbar_map.get("env") is None:
                self.cbar_map["env"] = self.fig.colorbar(
                    im, ax=self.axes["env"], orientation="horizontal", location="bottom", pad=0.08
                )
    ...
    def add_decoratives(self, ax: plt.Axes, info: Dict[str, Any]):
        ...
        circle = Circle(
            (goal_loc[0], goal_loc[1]), radius=goal_radius, color="black", fill=True, alpha=0.8
        )

        ax.add_patch(circle)
        ax.text(
            goal_loc[0],
            goal_loc[1] - goal_radius - 1.5,
            "goal",
            ha="center",
            va="bottom",
            fontsize=10,
        ...
        if nom_controller:
            ...
            if trajectories is not None:
                ...
                ax.plot(
                    weighted_traj[:, 0], weighted_traj[:, 1], color="purple", linewidth=2, zorder=10
                )
    ...
    def plot_velocity(self, ax: plt.Axes, info: Dict[str, Any]):
        ...
        if not ax.lines:
            ...
            env = info.get("env")
            ax.set_ylim(env.robot_params.action_min[0], env.robot_params.action_max[0])
    ...
    def plot_angular_velocity(self, ax: plt.Axes, info: Dict[str, Any]):
        ...
        if not ax.lines:
            ...
            env = info.get("env")
            ax.set_ylim(env.robot_params.action_min[1], env.robot_params.action_max[1])
    ...
    def reset(self):
        ...
        if has_preds:
            gs = self.fig.add_gridspec(
                3, 3, height_ratios=[0.4, 0.4, 0.4], width_ratios=[1.2, 0.6, 1.2]
            )
            ...
            gs = self.fig.add_gridspec(
                3, 2, height_ratios=[0.33, 0.33, 0.33], width_ratios=[1.5, 0.7]
            )
            self.axes["env"] = self.fig.add_subplot(gs[0:, 0])
            self.axes["risk"] = self.fig.add_subplot(gs[0, 1])
            self.axes["velocity"] = self.fig.add_subplot(gs[1, 1])
