...
@hydra.main(version_base=None, config_path="../../configs/agents", config_name="dubins")
def run_tests(cfg) -> None:
    print("🧪 Iniciando pruebas unitarias de DubinsRobot...")

    # Cargar y validar configuración usando el esquema de RobotParams y Pydantic
    from omegaconf import OmegaConf

    cfg_dict = OmegaConf.to_container(cfg, resolve=True)
    robot_params = RobotParams(**cfg_dict)

    # Definir límites de estado específicos para el test
    robot_params.state_max = np.array([50.0, 50.0, 2 * np.pi], dtype=np.float32)
    robot_params.state_min = np.array([0.0, 0.0, 0.0], dtype=np.float32)

    robot = DubinsRobot(robot_params)

    # 1. Test bound_state & filter_action
    # Filtrado dinámico según límites de acción en configuración
    v_max = robot_params.action_max[0]
    w_max = robot_params.action_max[1]
    assert np.allclose(
        robot.filter_action(np.array([v_max + 1.0, w_max + 1.0])), np.array([v_max, w_max])
    )
    assert np.allclose(
    ...
    # Espera propagación según robot_params.dt (asume dt = 0.1 en configuración)
    expected_x = 10.0 + 2.0 * float(robot_params.dt)
    assert np.allclose(next_states.numpy(), np.array([[expected_x, 10.0, 0.0]]), atol=1e-3)
    ...
    h_vals = robot.cbf_h_function(
        ego_states, neighbors, d_safe=1.5, k1=2.0, dt=float(robot_params.dt), t=0
    )
    ...
    neighbors_offset = [torch.tensor([11.0, 10.3, np.pi], dtype=torch.float32)]
    ...
    # grad_x < 0, grad_y = 0, grad_theta > 0 => safe_v = u_min[0]=0, safe_w = u_max[1]=w_max
    assert np.allclose(u_hj, np.array([robot_params.action_min[0], robot_params.action_max[1]]))
