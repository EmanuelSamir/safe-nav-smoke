...
import numpy as np
import torch
from pydantic import BaseModel, BeforeValidator, ConfigDict, model_validator, Field, AliasChoices
...
NpArray = Annotated[np.ndarray, BeforeValidator(to_ndarray)]


class RobotParams(BaseModel):
    model_config = ConfigDict(extra="forbid", arbitrary_types_allowed=True)

    name: str = Field(default="dubins2d", validation_alias=AliasChoices("name", "type"))
    action_dim: int = 2
    state_dim: int = 3

    action_max: NpArray = np.array([3.0, 4.0], dtype=np.float32)
    action_min: NpArray = np.array([0.0, -4.0], dtype=np.float32)
    state_max: NpArray = np.array([80, 30, 2 * np.pi], dtype=np.float32)
    state_min: NpArray = np.array([0, 0, 0], dtype=np.float32)
    ...
    def robot_type(self) -> str:
        """Alias for backward compatibility with code using robot_type."""
        return self.name

    @model_validator(mode="after")
    def validate_dimensions(self) -> "RobotParams":
