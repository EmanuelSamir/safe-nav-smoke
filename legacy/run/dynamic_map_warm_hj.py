...
def main(opts: dict = {}):
    ...
    nom_controller = NominalControl(robot_params, builder.params.resolution)
    nom_controller.set_odom(np.array([state["location"][0], state["location"][1], state["angle"]]))
    ...
    nominal_action_w = nom_controller.get_command()[0].item()
    nominal_action = np.array([robot_params.action_max[0], nominal_action_w])
    ...
    for t in tqdm(range(1,env_params.max_steps)):
        ...
        s = time()
        builder.build_map(learner, (t + 1)*env_params.clock)
        ...
        if t % hj_update_interval == 0 and not opts.get("safety_disabled"):
            if np.all(builder.failure_map == 1):
                ...
                # values = solver.solve(failure_mask.T, target_time=-2.5, dt=0.1, epsilon=0.0001)
                # failure_map = builder.failure_map.copy().T
                # failure_map[[0, -1], :] = 0
                # failure_map[:, [0, -1]] = 0
        ...
        if opts.get('render'):
            ...
            s = time() 
            im_ = builder.map_assets['continuous_map']
            if ax_pred.images:
                ax_pred.images[0].set_array(im_)
            else:
                pred_im = ax_pred.imshow(im_, cmap='gray', vmin=0, vmax=1.0, extent=[0, env_params.world_x_size, 0, env_params.world_y_size], origin='lower', zorder=-10)
...
if __name__ == "__main__":
    ...
    for i in range(samples):
        ...
        res = main(opts={'render': "human", 'seq_filename': f'result_attempt_10_tmp_{i+1}'})#, "safety_disabled": False, 'seq_filename': f'mppi_{i}'})
        df = pd.DataFrame(res, index=[0])
        time_end = time()
        # print(f'Time taken: {time_end - time_start} seconds')
        print('--------------------------------')
        # Verificar si el archivo existe
        file_path = 'misc/results_proposal_10_tmp.csv'
