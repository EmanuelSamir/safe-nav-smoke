...
class UnicycleRobot(Robot):
    ...
    def bound_state(self, state: np.ndarray) -> np.ndarray:
        ...
        state[3] = np.clip(
            state[3], self.robot_params.action_min[0], self.robot_params.action_max[0]
        )
        return state
...
if __name__ == "__main__":
    ...
    robot_params = RobotParams(
        ...
        state_max=np.array([50.0, 30.0, 2 * np.pi, 5.0]),
        action_min=np.array([-3.0, -2.0]),  # [a_min, omega_min]
        action_max=np.array([3.0, 2.0]),  # [a_max, omega_max]
        dt=0.1,
    )

    robot = UnicycleRobot(robot_params)
    robot.reset(np.array([10.0, 5.0, 0.0, 0.0]))  # start at rest
