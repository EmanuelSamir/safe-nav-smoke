...
def hocbf_safe_control(state_batch: torch.Tensor) -> torch.Tensor:
    ...
    u_safe = torch.where(u_safe_norm > AMAX, (u_safe / u_safe_norm) * AMAX, u_safe)
...
def main():
    ...
    # Configure modular DualGuard with HOCBF
    mppi = DualGuard(
    ...
    for step in tqdm(range(max_steps)):
        # Compute optimal shielded MPPI command with time tracking
    ...
    # ==========================================================
    #  PERFORMANCE METRICS PRINTS
    # ==========================================================
    total_time = np.sum(control_times)
    mean_time = np.mean(control_times) * 1000.0  # ms
    std_time = np.std(control_times) * 1000.0  # ms
    max_time = np.max(control_times) * 1000.0  # ms
    frequency = 1.0 / np.mean(control_times)  # Hz

    print("\n" + "=" * 50)
    print("📈 DUALGUARD MPPI PERFORMANCE STATS")
    print("=" * 50)
    print(f"• Total steps run:       {len(control_times)}")
    print(f"• Total planning time:   {total_time:.4f} seconds")
    print(f"• Mean step duration:    {mean_time:.3f} ms (± {std_time:.3f} ms)")
    print(f"• Max step duration:     {max_time:.3f} ms")
    print(f"• Planning frequency:    {frequency:.2f} Hz")
    print("=" * 50 + "\n")
    ...
    ax.grid(True)
    ax.set_title(
        "Shielded MPPI with High-Order Control Barrier Functions (HOCBF)\n(Double Integrator System)"
    )
